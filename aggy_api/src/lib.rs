    ...
    T: AuthenticatedEndpoint,
{
    ...
    async fn handle(
        ...
        self.handle(cx, accessing_user, request).await
    }
}
    ...
    for<'a> &'a Self::Error: Into<StatusCode>,
{
    ...
    type HttpRequest: axum::extract::FromRequest<SharedContext, axum::body::Body>
        + Send
        + Sync
        + 'static;
...
impl<T> axum::handler::Handler<T::Request, SharedContext> for EndpointWrapper<T>
where
    ...
    for<'a> &'a T::Error: Into<StatusCode>,
{
    ...
    fn call(self, req: hyper::Request<hyper::Body>, cx: SharedContext) -> Self::Future {
        self.http(req, cx)
    }
}

impl<T> From<EndpointWrapper<T>> for axum::Router<SharedContext>
where
    T: HttpEndpoint + Clone,
    T::Error: serde::Serialize,
    for<'a> &'a T::Error: Into<StatusCode>,
...
where
    ...
    T::Request: axum::extract::FromRequest<SharedContext, axum::body::Body>,
